...
public class ReviewController extends CrudController<Review, ReviewDto, Long> {
    ...
    public ResponseEntity<List<ReviewDto>> filter(
            ...
            @Parameter(description="Approvement status of the review to retrieve.", required=true)
    ...
    public ResponseEntity<ReviewDto> approve (
            ...
            @PathVariable Long id ,
            @Parameter(description="User id of the review to retrieve.", required=true)
            @RequestParam("user_id") Long userId) {
        return ResponseEntity.ok(
                reviewDtoMapper.fromDomain(reviewService.approve(
